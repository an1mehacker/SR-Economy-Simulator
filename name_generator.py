...
def generate_peleng_name():
    starts = {
        "sibilant": ["Ukh", "Shu", "Sha", "Shai", "Ches", "Che", "Chush", "Tsa", "Tush", "Bush"],
        "plosive": ["Ayk", "Akha", "Apac", "Ikht", "Nat", "Ukh"],
...
def generate_peleng_name3():
    starts = ['Ash', 'Che', 'Ti', 'Tsa', 'Sip', 'Lyak', 'Sut', 'Kas', 'Tsla', 'Bush', 'Ya', 'Ri', 'Ra', 'Pen', 'Ro', 'Tsai',
     ...
     'Tsu', 'Let', 'A', 'Nats', 'Ret', 'Lo', 'Ih', 'Ye', 'Les', 'Ni', 'Uh', 'Li', 'Its', 'Lip', 'Lel', 'Shai', 'La',
     'Sit', 'Chiv', 'Na', 'Te', 'Nu', 'Lok', 'Ha', 'Ray', 'Kin', 'Ta', 'Lik', 'Al', 'Ke', 'Uk', 'Chi', 'Nui', 'Suh',
     'Shat', 'Le', 'Ai', 'Ku', 'He', 'Lep', 'Shu', 'Nel', 'Sa', 'Peh', 'Ches', 'Sui', 'Ruh', 'Shek', 'Cha', 'Nuk',
     'Sei', 'Tsot', 'Ka', 'U', 'Sheh', 'Tsi', 'Tse']

    mids = ['pak', 'pek', 'kay', 'ses', 'sosh', 'si', 'pei', 'its', 'sih', 'sek', 'lot', 'akk', 'kuk', 'shu', 'sos',
            'tsay', 'tsen', 'chut', 'pa', 'ish', 'shtoch', 'llyas', 'int', 'han', 'tsom', 'chin', 'pra', 'ats', 'tsi',
            ...
            'tik', 'hi', 'vin', 'chet', 'tsin', 'ship', 'ko', 'hit', 'shikk', 'chik', 'tsik']
    ...
    ends = ['li', 'sta', 'cha', 'pek', 'kah', 'is', 'la', 'lak', 'yecha', 'kla', 'kesh', 'ska', 'hats', 'ha', 'an', 'shi',
            ...
            'lish', 'ak', 'kish', 'ek', 'shik', 'sis', 'plits', 'kiish', 'kan', 'ah', 'sent', 'lan', 'cak', 'ish',
            'chik', 'pan', 'tank', 'shan', 'san', 'han', 'tash', 'ta', 'sha', 'nak', 'ik', 'nash', 'pru', 'skish',
            'kash', 'nah', 'tak', 'ats']

    syllable_count = random.randint(1, 4)

    if syllable_count == 1:
        return random.choice(list(starts))
    elif syllable_count == 2:
        return random.choice(list(starts)) + random.choice(list(ends))
    elif syllable_count == 3:
        return random.choice(list(starts)) + random.choice(list(mids)) + random.choice(list(ends))
    else:  # syllable_count == 4
        return (
                random.choice(list(starts)) +
                random.choice(list(mids)) +
                random.choice(list(mids)) +
                random.choice(list(ends))
        )


def generate_peleng_name2():
    starts = ["Tush", "Akha", "Ally", "Apac", "Bush", "Che", "Chush", "Ches", "Ikht", "Lek", "Lya", "Nat", "Shai", "Tsa", "Ukh", "Shu"]
    mids = ["ka", "sha", "chi", "tak", "kish", "lak", "sish", "pash", "pek", "tsak", "khak", "shka"]
    ends = ["ch", "tka", "sha", "chan", "ish", "an", "ak", "ash" "osh", "ska", "tsis", "kach", "shak", "shik", "shok"]

    first = random.choice(starts) + (random.choice(mids) + random.choice(ends) if random.random() < 0.7 else random.choice(ends))
    last = random.choice(starts) + (random.choice(mids) + random.choice(ends) if random.random() < 0.7 else random.choice(ends))
    return f"{first} {last}"
...
def generate_human_name():
    ...
    if ethnicity == "American-European":
        ...
        first = random.choice(
            ["Ivan", "Anya", "Mikhail", "Katerina", "Boris", "Nina", "Dmitri", "Tatiana", "Alexei", "Irina"])
        last = random.choice(
...
def split_syllables_into_sets(syllable_names):
    starts = set()
    mids = set()
    ends = set()

    for syllables in syllable_names:
        if not syllables:
            continue
...
for _ in range(100):
    ...
    print(generate_peleng_name3() + " " + generate_peleng_name3())
